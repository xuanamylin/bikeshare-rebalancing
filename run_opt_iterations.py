...
sa_hyperparameters = {'temp_schedule': 50, 
                      'K': 50, 
                      'alpha': 0.92,
                      'iter_max': 5000, 
                      'temp': 100, 
...
# Get distance matrix (min)
time_mtrx = distance[(distance['from'].isin(top_station_ids)) 
                          & (distance['to'].isin(top_station_ids))].copy()
time_mtrx = pd.pivot_table(time_mtrx, values='distance', index='from', columns='to', aggfunc=np.sum)
time_mtrx = time_mtrx / 600

# Get expected demand
case = traffic[(traffic['date'] == sample_date) 
                    & (traffic['time_bin'] == sample_bin)].copy()
case['inflow_pred'] = model_inflow.predict(case[features]).round()
case['outflow_pred'] = model_outflow.predict(case[features]).round()
case['net_flow'] = case['inflow_pred'] - case['outflow_pred']
case = pd.merge(case, sensor_info, on = 'station_id', how = 'outer')
...
def output_aco_solution(aco_output, aco_opt, actual_list, expected_list):
    ...
    aco_route = [p[0] for p in best_path] + [aco_opt.start_station]
...
abs(expected.sum() - actual.sum()) --> unsatisfiable
satisfiable 
...
def run_cases(test_cases, N, filepath_template, data_source_dict, 
    ...
    for test in test_cases:
        ...
        pickle_name = filepath_template.format(N, sample_date, sample_bin, version)
...
if __name__ == '__main__':
    ...
    filepath_template = output_path + "solution_{}stops_{}_{}_{}.pickle"
    
    # Run and Save Results
    N = 25
    version = 'test'
    result_df = run_cases(inpt2, N, filepath_template, data_sources, 
                          sa_hyperparameters, aco_hyperparameters, version, debug = True)
    result_df.to_csv(output_path + '{}stop_cases_agg_{}.csv'.format(N, version), index = False)
